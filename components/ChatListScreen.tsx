...
const MOCK_CHATS: ChatItem[] = [
  { id: '1', name: 'Alex Rivera', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Alex', lastMessage: 'See you at 8? Don\'t forget the docs!', time: '12:45 PM', unreadCount: 2, type: 'user', isOnline: true },
  { id: '2', name: 'Design Team 🚀', avatar: 'https://api.dicebear.com/7.x/identicon/svg?seed=Design', lastMessage: 'Jordan: New assets uploaded...', time: '10:12 AM', unreadCount: 0, type: 'group' },
  { id: '3', name: 'Tech News Weekly', avatar: 'https://api.dicebear.com/7.x/initials/svg?seed=TN', lastMessage: 'Apple announces new M3 Max...', time: '9:30 AM', unreadCount: 5, type: 'channel', isVerified: true },
  { id: '4', name: 'Sarah Jenkins', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Sarah', lastMessage: 'Meeting rescheduled.', time: 'Yesterday', unreadCount: 0, type: 'user' },
  { id: '5', name: 'Marcus Aurelius', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Marcus', lastMessage: 'Thanks for the feedback!', time: 'Tuesday', unreadCount: 0, type: 'user' },
];
...
const ChatListScreen: React.FC<ChatListProps> = ({ onSelectChat, onDiscovery, onNavClick, activeScreen, hideFooter, selectedChatId }) => {
  ...
  return (
    <div className="h-full flex flex-col pt-6 border-r border-white/5">
      <header className="px-6 mb-4">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Messages</h1>
          <button className="text-primary material-icons-round bg-primary/10 p-2 rounded-xl">edit_note</button>
        </div>
        ...
        <div className="flex justify-between relative border-b border-white/5">
          {[ChatCategory.ALL, ChatCategory.GROUPS, ChatCategory.CHANNELS].map(tab => (
            <button 
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-3 text-xs font-bold capitalize transition-colors ${activeTab === tab ? 'text-primary' : 'text-slate-500'}`}
            >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex justify-between items-center gap-2">
                  ...
                  {chat.unreadCount > 0 && (
                    <span className="bg-primary text-white text-[10px] font-bold h-4 min-w-4 px-1 rounded-full flex items-center justify-center">
                      {chat.unreadCount}
                    </span>
                  )}
                </div>
